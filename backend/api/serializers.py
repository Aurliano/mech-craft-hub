...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = [
            'id', 'username', 'email', 'phone', 'password', 'hcaptcha_token', 'cf_turnstile_response',
    ...
    def validate(self, data):
        """Validate captcha - priority: Turnstile > hCaptcha > fallback"""
        ...
        # If no Turnstile but hCaptcha token is provided and valid, we're good
        if hcaptcha_token:
            return data
        
        # If no cloud captcha, validate fallback captcha
    ...
    def create(self, validated_data):
        ...
        hcaptcha_token = validated_data.pop('hcaptcha_token', None)
        turnstile_token = validated_data.pop('cf_turnstile_response', None)
        ...
        # Log successful captcha attempt
        if hcaptcha_token:
...
class LoginSerializer(serializers.Serializer):
    ...
    hcaptcha_token = serializers.CharField(required=False, allow_blank=True)
