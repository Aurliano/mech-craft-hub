...
const AnalysisSimulation = () => {
    ...
    (async () => {
      try {
        ...
        if (analysis?.id) {
          ...
          console.error('Analysis service not found in available services');
          setAnalysisServiceId(null);
          setServiceError('سرویس تحلیل و شبیه‌سازی در سیستم یافت نشد. لطفاً با پشتیبانی تماس بگیرید.');
        }
      } catch (error) {
  ...
  const AnalysisForm = () => (
    <div className="space-y-6">
      ...
      {!analysisServiceId && !serviceError && (
